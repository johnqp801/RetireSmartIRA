...
- **RUN TESTS WITH `tools/run-tests.sh`, NOT `xcodebuild` DIRECTLY.** The wrapper now exists (written 2026-08-05, on main and merged into this branch) and CLAUDE.md points at it. It derives `-project` from its own location so it cannot build a sibling worktree, and it prints about six lines instead of tens of thousands. Still run it in the FOREGROUND with `timeout: 600000`; five agents each lost a full turn by backgrounding a build, and the wrapper removes the reason to.
  It also refuses a false green: zero tests executed exits non-zero rather than reporting PASS, and when the only failure is the known `MultiYearPerfTests` flake it re-runs that suite in isolation to CHECK the claim rather than waving it through.
