...
**Implications:** (a) future LinkedIn posts should feature real app screenshots (present-value toggle, ladder, couples heat-map); (b) the present-value landing page hero should likely swap the abstract bars for a real screenshot of the present-value toggle before deploy; (c) the WSJ widow-penalty pitch and HumbleDollar-type placements are the highest-leverage install channels and should be prioritized over polishing concept posts.

**CONFIRMED 2026-07-22:** the Jul 4 spike = John's HumbleDollar guest article **"A $30,000 Mistake"** (https://humbledollar.com/2026/07/a-30000-mistake/, pub 2026-07-04) — the ACA/IRMAA pincer content, byline plugs RetireSmartIRA. Two follow-on notes: (1) **Correction** — Claude earlier CUT a "wrote a $30k piece for HumbleDollar" line from the LinkedIn brief as unverified/fabricated; it was TRUE (memory just lacked the article). That reference is real and usable. (2) The byline surfaced verified bio facts — John **founded GT Nexus (supply-chain SaaS, acquired by Infor 2015)** — the answer to the recurring "earlier career" question; now safe to use in builder-story copy and the WSJ pitch. Saved to auto-memory `user-bio-verified.md` + `download-driver-attribution.md`.
...
## 2026-07-26: Selectable Roth tax-payment source COMMITTED TO A CUSTOMER for V2.3, early August

**Trigger:** Support email from Joan Menard (found the app via the HumbleDollar "Widow Tax" article published 2026-07-25). Couple aged 66/67, no earned income expected in 2027, no cash or taxable accounts, $665/mo Social Security, fully dependent on IRA withdrawals. She read Ed Slott, believes funding conversion tax from IRA dollars is discouraged, modeled it herself, and asked only whether the app models her situation correctly (explicitly NOT asking for advice).

**Verified state before replying (code audit, do not re-derive):** Multi-year ALREADY handles her case correctly and by default. `TaxPaymentSource` (`MultiYearAssumptions.swift:10-13`) has two cases, `.taxableThenGrossUp` (default at `:41`, `:62`, `:103`) and `.external` (legacy/back-compat). With zero taxable accounts the entire conversion tax is funded from traditional via a real fixed-point gross-up solve, 3 iterations / $1 tolerance (`ProjectionEngine.swift:864-902`), debited older-spouse-first (`:914-925`), surfaced as a `.traditionalWithdrawal` action and as `YearRecommendation.taxFundingWithdrawal` (`:1068`), and fed into the IRMAA 2-year lookback (`:959-982`) and the NIIT threshold (`:991-995`).

**What is actually missing is the CHOICE, not the capability.** There is no UI control for `taxPaymentSource` anywhere; the multi-year behavior is effectively hardcoded. Initial instinct was to tell Joan "multi-year does not handle where the Roth tax comes from," which was wrong and would have sent her away from the one screen that answers her question. Corrected before sending.

**Commitments made in writing to a customer (must be honored or the date corrected):**
1. **Selectable tax payment source in V2.3, early August 2026.**
2. Fixing the Social Security taxability gap below, described to her as "on my list."

**Precedent this repeats:** the selectable optimizer objective promised to Tim that did not exist. See `optimizer-objective-not-selectable`. Rule stands: do not claim a capability in copy or email until built. This time the claim is dated, so V2.3 scope must carry it.

**Five findings from the same audit (V2.3 backlog, severity order):**
1. **`underfunded` is computed and never surfaced.** `ProjectionEngine.swift:939` records tax that neither taxable sales nor the gross-up could cover; no view, PDF, or CPA briefing reads it. An infeasible plan renders as valid. Hits exactly the zero-outside-cash profile Joan describes. Smallest fix, highest severity, value already exists.
2. **Single-year and multi-year disagree by default.** Single-year `rothConversionWithholdingMode` defaults to `.paidFromOutside` (`DataManager.swift:145`); multi-year defaults to gross-up-from-IRA. Two screens give a no-cash user different answers with no explanation. The two systems are not connected.
3. **SS taxability frozen pre-gross-up.** `taxableSS` computed at `ProjectionEngine.swift:626` and never recomputed after `dW`, so the fixed point under-solves whenever the tax-paying withdrawal pushes more benefit into taxability. This is the limitation disclosed to Joan.
4. **ACA MAGI excludes the gross-up.** `acaMagiValue` at `:681` is pre-gross-up and reported verbatim at `:1044`, overstating subsidies. Does not affect Joan at 66/67; affects every pre-65 conversion user.
