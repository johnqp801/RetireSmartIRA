...
struct ConstraintAcceptorTests {
    ...
    func irmaaTier1HitAtThreshold() {
        // 2026 single tier-1 starts at MAGI > $109,001
        let path = [year(year: 2026, agi: 110_000, irmaaMagi: 110_000, taxableIncome: 95_000)]
        let acceptor = ConstraintAcceptor()
        let hits = acceptor.detect(path: path, filingStatus: .single, householdSize: 1)
    ...
    func detectAcrossMultipleYears() {
        let path = [
            year(year: 2026, agi: 130_000, irmaaMagi: 130_000, taxableIncome: 110_000),  // IRMAA tier 1
            year(year: 2027, agi: 130_000, irmaaMagi: 130_000, taxableIncome: 110_000),  // IRMAA tier 1 again
