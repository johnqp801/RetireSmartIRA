...
@MainActor struct EndToEndTaxValidationTests {
    ...
    func b_stateTax() {
        let dm = makeScenarioB()
        // Updated 2026-05-18 — PA/IL/MS/etc. retirement exemption now applied per state config (1.8.1.1 PA fix).
        // NY $20K IRA-withdrawal exemption now applied to scenario-level retirement
        // distributions (computed RMDs) per the build-38 engine-wiring fix.
        // Previously the exemption only matched `.rmd`-typed IncomeSource rows,
        // missing $35,283 of auto-calc RMD.
    ...
    @Test("B: Total tax ≈ $25,306")
    func b_totalTax() {
        let dm = makeScenarioB()
        // Updated 2026-05-18 — PA/IL/MS/etc. retirement exemption now applied per
        // state config (1.8.1.1 PA fix). NY $20K IRA-exemption now applied to
        // auto-calc RMDs per the build-38 engine-wiring fix (see b_stateTax).
        // Previous total was $26,475.80; state tax dropped by $1,170.
        #expect(isClose(dm.scenarioTotalTax, 25_305.80, tolerance: 2.0))
    }
}
...
@MainActor struct StressTestD_MFJNY {
    ...
    func d_stateTax() {
        let dm = makeScenarioD()
        // Updated 2026-05-18 — PA/IL/MS/etc. retirement exemption now applied per
        // state config (1.8.1.1 PA fix). NY $20K IRA-withdrawal exemption now
        // applied to scenario-level $86,792 auto-calc RMD per the build-38 wiring fix.
    ...
    func d_totalTax() {
        ...
        // Updated 2026-05-18 — PA/IL/MS/etc. retirement exemption now applied per
        // state config (1.8.1.1 PA fix). NY $20K IRA-exemption now applied to
        // auto-calc RMDs per the build-38 engine-wiring fix (see d_stateTax).
