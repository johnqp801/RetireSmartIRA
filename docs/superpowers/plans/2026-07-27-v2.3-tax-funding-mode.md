...
```swift
            if underfundedTax > 0 { sawInfeasibleYear = true }
```

Order matters: a year that is itself infeasible reports `dependsOnInfeasibleYear == false` unless an *earlier* year already failed.
